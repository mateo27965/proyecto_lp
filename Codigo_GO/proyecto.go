...
func (se *busqueda) verHistorialBusqueda() {
	if se.UsuarioActual != nil {
		...
		for i := range se.UsuarioActual.registroBusqueda {
			fmt.Printf("%-40s || %-30s || %-20s || %-25d || %-15s\n", se.UsuarioActual.registroTexto[i], se.UsuarioActual.registroBusqueda[i], fmt.Sprintf("%.2f s", se.UsuarioActual.registroTiempo[i]), se.UsuarioActual.registroCantApariciones[i], se.UsuarioActual.registroAlgoritmo[i])
		}
	} else {
