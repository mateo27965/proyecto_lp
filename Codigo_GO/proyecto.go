...
func (se *busqueda) verHistorialBusqueda() {
	if se.UsuarioActual != nil {
		...
		fmt.Printf("%-37s || %-17s || %-15s || %-17s || %-15s\n", " Texto elegido", "Palabra/oracion", "Tiempo de duración", "Cant. apariciones", "Algoritmo")
		...
		for _, registro := range registros {
			fmt.Printf("%-37s || %-17s || %-15s || %-17d || %-15s\n", registro.Texto, registro.Palabra, fmt.Sprintf("%.14f s", registro.Duracion), registro.CantApariciones, registro.Algoritmo)
		}
	} else {
...
func main() {
	...
	for {
		...
		fmt.Println("3. Ver historial de búsquedas")
		fmt.Println("4. Salir")
		...
		case "2":
			fmt.Println("\n=== BUSCAR ===")
			fmt.Println("1. Fuerza Bruta")
			fmt.Println("2. Knuth-Morris-Pratt")
			fmt.Println("3. Boyer-Moore")
			fmt.Print("Ingrese la opción: ")
